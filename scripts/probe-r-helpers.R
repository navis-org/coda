...
#
# The edge ids are **numeric** and the seeds are text, which is the mixture a real document
# produces: `neuprint_connection_table` returns numeric body ids, and a seed arrives from a Coda
# column, which is character. The cast inside the helper is what reconciles them — and the two
# languages fail differently without it, which is why both probes carry the case. `bind_rows`
# here *errors* on `<character>` against `<double>`; `pd.concat` there silently returns the seed
# twice.
conn <- data.frame(
  preId = c(1, 1, 2),
  preType = c("A", "A", "B"),
...
# The regression the cast exists for: a seed and its own edge end are one neuron.
check("endpoints: a seed and its edge end are one neuron, not two",
      nrow(eps) == 4L, as.character(nrow(eps)))
check("endpoints: every id is text", is.character(eps$neuronId), class(eps$neuronId))
