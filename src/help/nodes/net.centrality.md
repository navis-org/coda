...
It runs on **Run** only. Betweenness walks every link once per node, so on a big network it is minutes, not milliseconds — see `Sample`.

## The five measures

- **Betweenness** — how many of the shortest routes between other nodes pass through this one. High means a bottleneck.
...
- **Eigenvector** — the same idea in its classical form, and off by default. On a network where signal mostly flows forwards it gives zero to everything not part of a loop, which is most of it: a column of zeros that looks like a bug and is the measure working as defined.
...
| `diameter` | The longest of those shortest routes. **Empty whenever `Sample` was used** — a sampled maximum is only ever a lower bound. |
| `reachable` | Of all ordered pairs of nodes, the share that can get from one to the other. Low is normal for a filtered connectome. |
...
`Weighted paths` changes what "short" means. Off, a path is a count of steps. On, a link's length is 1/weight, so a strong connection is a short path. Parallel links are added together first, so the answer does not depend on whether `Merge parallel links` was ticked upstream in [Build Network](#net.build).

> [!NOTE] Only turn on what you will read
> Each measure off is a column that is not offered rather than a column of blanks. Betweenness and
> closeness share one sweep, so having both costs barely more than one; the others are cheap next
> to it.
...
`Resolution` above 1 makes Louvain find more, smaller communities; below 1, fewer and bigger. `Damping` is PageRank's chance of following a link rather than jumping somewhere random — 0.85 is the standard value.
