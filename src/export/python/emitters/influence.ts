...
registerHelper({
  ...
  source: [
    ...
    '    # feedback.',
    '    # Counted from a hop that is *in the table*: the extremes are taken over the ribbons',
    '    # `flow_floor` kept, not over every one the walk produced. A four-hop budget that runs',
    '    # out of graph after two would otherwise number its layers 2 and 3, leaving an empty',
    '    # column in front of them that a reader cannot tell from a filter -- and a hop whose',
    '    # every ribbon fell under the floor is just as absent. An upstream walk conserves mass,',
    '    # so a deeper hop spreads the same total over more cell-type pairs and the floor takes',
    '    # whole trailing hops before it thins a shallow one.',
