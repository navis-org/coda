...
export interface PortDef {
   ...
   *
   * Not the same statement as `required: false`. Three optional ports normally *compose* —
   * `neuron.connectivity`'s `neurons` and `labels` both add something — and every automatic
   * wiring pass in this codebase reads them that way, filling each port that has a compatible
   * source. `core.embed` is the first node where they do not: a score matrix, a feature table
   * and a k-NN table are three ways of arriving at one k-NN graph, and nothing makes "the
   * matrix wins" a defensible rule.
   *
   * Found by building it. The node guide's **Open in a workflow** link is generated by
   * `wizard/demo.ts`, which appends the node to whatever port already carries a compatible
   * value — so for `core.embed` it wired all three from one neuron table and the demo opened
   * on the node's own refusal. `demo.test.ts` holds the general form.
   *
   * It is a **declaration a node makes**, not a constraint the canvas enforces: wiring a second
   * one by hand is reported by the node's own `validate`, where the message can say which port
   * to disconnect and why. The same split `producedBy` makes — a fact the ports cannot state,
