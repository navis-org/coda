...
import type { CompareInput } from '../../nodes/lib/edgeComparison'
import {
  COUNTS_SCHEMA,
  MAX_COMPARED_DATASETS,
  compareDatasetName,
  resolveDatasetNames,
  compareConnectivity as compareEdgeTables,
  compareParamsFrom,
  comparisonSchema,
} from '../../nodes/lib/edgeComparison'
import { MAPPER_LABELS_SCHEMA } from '../../nodes/lib/typeMapping'
import { ID_COLUMN_NAME } from '../../core/ids'
import { portIdAt } from '../../core/ports'
import { repeatGroups, repeatParamId, repeatParams } from '../../nodes/lib/repeatParams'

/**
 * The arity, declared once and handed to both the param list and `repeatParams`.
 *
 * The registry reads the group's range off this object; so does the picker loop. A bare
 * `MAX_COMPARED_DATASETS` in the second place is the copy `registry.ts` removed from `PortGroupDef`.
 */
...
export const compareConnectivityNode = packNode({
