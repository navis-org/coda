 ...
 *
 * Only the list scrolls (`ShelfList`), so the note and Import stay on screen; `recipes.md` has the
 * measurement.
 */
...
import { SHELF_FILTER_FROM, ShelfList, ShelfRow } from './ShelfRow'
...
function RecipesPanel({ insertAt }: { insertAt: () => Point }) {
  ...
  // A long shelf holds its height (`.recipe-dialog--long`), and it is long when it has a filter.
  const long = recipes.length >= SHELF_FILTER_FROM

  return (
    <Modal
      className={`overlay__panel recipe-dialog${long ? ' recipe-dialog--long' : ''}`}
      labelledBy={titleId}
      onClose={close}
    >
      ...
      <div className="sources__body recipe-dialog__body recipe-dialog__body--list">
        ...
        <ShelfList entries={recipes} noun="recipe">
          {(entry) => (
            <ShelfRow
              name={entry.name}
