...
function ValuePreviewInner({
  ...
  const shared: SharedViewerProps = {
    compact,
    ...(baseName ? { baseName } : {}),
    ...(onExpand ? { onExpand } : {}),
    ...(onError ? { onError } : {}),
  ...
  /*
   * The params every entry reads, each declared default filled (`withDefaults`), so an entry
  ...
  /*
   * Up here rather than in the table fallback below, because this component returns early a
  ...
  if (!value) {
    return <ViewerEmpty>No result yet — run the graph to see output.</ViewerEmpty>
  }
  ...
  if (
    ...
    return <ViewerEmpty>{describeValue(value)}</ViewerEmpty>
  }
...
/** What every viewer takes, so a new one cannot ship without export or expand. */
...
/** What a `VIEWERS` entry is handed: `ValuePreview`'s own props, and the readers every entry shares. */
type ViewerProps = Pick<
  ValuePreviewProps,
  'node' | 'value' | 'ctx' | 'inputValues' | 'onParamChange' | 'onSelectionChange'
> & {
  params: ParamValues
...
interface ViewerEntry {
  /** Drawn before the "No result yet" guard, from the node's inputs — see `VIEWERS`. */
  withoutValue?: boolean
  /**
   * Draws from its inputs too, so its card shows a preview before its own value — but after the
 ...
 *
 * **An entry declines by returning `undefined`** — its input or its value is not the kind it
 * draws — and dispatch carries on to the guard and the value-kind fallbacks. Entries are called,
 * not mounted, so none adds a component to the tree.
 *
 * `CodaNodeView` asks the table which cards have a preview before their own value
 * (`drawsFromInputs`): the `withoutValue` entries, and the ones marked `readsInputs`.
 */
const VIEWERS: Record<string, ViewerEntry> = {
  /*
   * Drawn without a value, and that is the whole reason this node renders at all.
   *
   ...
   * undefined forever — after the guard its entry is unreachable and the card shows "No result
  ...
  'out.datasetSummary': {
    withoutValue: true,
    render: ({ ctx, params, choice, shared, inputValues, onParamChange }) => {
  ...
  // No outputs, so no value ever: `out.datasetSummary`'s case exactly.
  'out.rois': {
    withoutValue: true,
  ...
  /*
   * Drawn without a value, like `out.rois`, and for a sharper reason.
   *
   * This node's own output is the *selection* — empty until it runs — while the scene is on its
   * inputs. After the guard, a 3D View could only draw after its own evaluation, which is one
  ...
  'out.viewer3d': {
    ...
    render: ({
      ...
      if (!(skeletons || meshes || points || volumes)) return undefined
      return (
        <LazyViewer3D
  ...
  /*
   * Drawn without a value, on `out.viewer3d`'s terms: the card is drawn from the *input*
  ...
  'net.metrics': {
    withoutValue: true,
    render: ({ ctx, params, shared, inputValues, onParamChange }) => {
      if (!isNetworkValue(inputValues?.in)) return undefined
      return (
    ...
    render: ({ params, choice, shared, inputValues, onParamChange, selection }) => {
      ...
      return (
        <TopologyViewer
          ...
          onColorBy={(next) => onParamChange?.('colorBy', next)}
          ...
          onGrouping={(next) => onParamChange?.('grouping', next)}
          direction={choice('direction')}
          onDirection={(next) => onParamChange?.('direction', next)}
          partnerQuery={String(params.partnerQuery)}
          onPartnerQuery={(next) => onParamChange?.('partnerQuery', next)}
          tab={choice('tab')}
          onTab={(next) => onParamChange?.('tab', next)}
          ...
          onSplitParam={(id, next) => onParamChange?.(id, next)}
  ...
  'out.dendrogram': {
    render: ({
      ...
      if (!isLinkageValue(value)) return undefined
  ...
  'out.heatmap': {
    render: ({ value, params, choice, shared, onSelectionChange, selection }) => {
      if (!isMatrixValue(value)) return undefined
      return (
        <HeatmapViewer
          matrix={value}
  ...
  'out.barChart': {
    render: ({ value, ctx, params, shared }) => {
      if (!isTableValue(value)) return undefined
...
/** Whether a node type's card has a preview before its own value, drawn from its inputs. */
export function drawsFromInputs(type: string): boolean {
  const entry = VIEWERS[type]
  return entry?.withoutValue === true || entry?.readsInputs === true
}
