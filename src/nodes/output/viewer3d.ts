...
registerNode({
  ...
  params: [
    ...
    ...colorParams({
      ...
      allowLiteral: true,
      allowHash: true,
      // `by value` gets a ramp, ends, a centre and a log — the same controls on all four sockets,
      // for the reason the encodings are peers at all. See `ColorParamOptions.valueScale`.
      valueScale: true,
      from: 'skeletons',
