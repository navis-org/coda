...
/**
 * Every operator, grouped by the column type it applies to, as one line for the catalogue.
 *
 * **Generated by asking `opsForDType`, not transcribed**, which is `filtersNote()`'s rule in
 * `nodes/query/findNeurons.ts` and for the same reason: a hand-written list is wrong the first time
 * somebody adds an operator, and wrong silently, since nothing fails to compile.
 *
 * It exists because the per-node `options:` line cannot cover the case that matters most. That
 * line describes a node *on the canvas*, and the commonest thing a plan does is **add** one —
 * at which point there is no node to describe and the catalogue's `(options depend on the
 * input)` is all the model has. Measured: told only that, a model wrote `op: "is"`, which is
 * the *label* of `eq`; told nothing at all about the vocabulary, it left `op` unset and
 * inherited the numeric default on a text column. Both apply, and both leave a warning on a
 * card the user then has to find.
 *
 * **The criterion, so the next case is recognised rather than re-measured: a note is worth
 * writing where the vocabulary is *closed and known to the type*.** These operators are, and so
 * are `SkeletonRouteId` and the synapse-unit tuple, which have the same gap and no note yet. The
 * other dynamic params — ROI names, materializations, discovered statuses, a file's columns —
 * have no static union to print, which is why the per-node `options:` line exists for them.
 */
export function operatorVocabulary(): string {
  const groups: Array<[string, DType]> = [
    ['a number column', 'i64'],
    ['a text column', 'str'],
    ['a boolean column', 'bool'],
  ]
  return [
    'Which operators are legal depends on the column. Write the value, never the label:',
    ...groups.map(
      ([label, dtype]) =>
        `  ${label}: ${opsForDType(dtype)
          .map((o) => o.value)
          .join(' | ')}`,
    ),
  ].join('\n')
...
/**
 * A schema with some columns folded in — kept where they already are, appended where they are new.
 *
 * Two rules, and the second is the one that gets got wrong. **A name is written over, never
 * beside**: two `degreeIn` columns give a picker downstream two answers to one question and the
 * second is the stale one. And **position is preserved**, because a column that moves to the end
 * whenever it is recomputed is a table whose columns reorder when nothing about it changed —
 * `TableViewer` draws `schema.columns` in order, the CSV export writes that order, and GraphML
 * key ids are generated by index over it.
 *
 * Here rather than in `networkOps`, where it was written and lived while it had one caller. It has
 * four now — `net.metrics`, `net.centrality`, `Carry fields` and `Points in Volumes` — and only
 * the *no-schema* answer was ever the network's, so a point cloud was importing `foldNodeColumns`
 * from the network ops file and then subtracting the `id` column it promises. `foldNodeColumns`
 * is that subtraction's opposite now: this plus the network's own default.
 */
export function foldColumns(schema: TableSchema, wanted: readonly ColumnSchema[]): TableSchema {
  return tableSchema(
    ...schema.columns.map(
      (existing) => wanted.find((c) => c.name === existing.name) ?? existing,
    ),
    ...wanted.filter((c) => !findColumn(schema, c.name)),
  )
}
