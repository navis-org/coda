...
| `data/geometryCache.test.ts`             | the session geometry cache: asks for exactly the missing ids, never calls the fetcher with nothing, hands back the same array rather than a copy, keeps two levels of detail apart, LRU by bytes, and the age a hit reports |
...
| `ui/viewers/liveRenderers.test.tsx`      | one live renderer per node: the inspector standing a WebGL viewer down, and a card dropping its preview while the overlay owns it |
