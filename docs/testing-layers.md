...
| `core/scheduler.test.ts`                 | hybrid eval, caching, invalidation, errors, targeted runs, and partial results — that `ctx.publish` is visible while a node runs, is replaced by the real result when it settles, and is dropped rather than left standing when it fails |
