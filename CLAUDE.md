...
- **`Find Neurons` asking nothing answers nothing, and that decision is at the node rather than at
  ...
  param holding JSON, so `catalogue.ts` carries a `FILTER_ROW_RULE` **generated by calling
  `encodeRows` and reading `ALL_ROW_OPS`** rather than transcribed — drift there is refused with a
